...
func RecordMetricsRemover(ctx context.Context, p metric.MeterProvider, totalRemoved int64) error {
	counter, err := p.Meter("eraser").SyncInt64().Counter(ImagesRemovedCounter, metric.WithDescription(ImagesRemovedDescription), metric.WithUnit("1"))
	if err != nil {
		return err
	}
...
func RecordMetricsScanner(ctx context.Context, p metric.MeterProvider, totalVulnerable int) error {
	counter, err := p.Meter("eraser").SyncInt64().Counter("vulnerable_images_run_total", metric.WithDescription("total vulnerable images"), metric.WithUnit("1"))
	if err != nil {
		return err
	}
...
func RecordMetricsController(ctx context.Context, p metric.MeterProvider, jobDuration float64, podsCompleted int64, podsFailed int64) error {
	duration, err := p.Meter("eraser").SyncFloat64().Histogram("imagejob_duration_run_seconds", metric.WithDescription("duration of imagejob"), metric.WithUnit("s"))
	if err != nil {
		return err
	}
	duration.Record(ctx, jobDuration)

	completed, err := p.Meter("eraser").SyncInt64().Counter("pods_completed_run_total", metric.WithDescription("total pods completed"), metric.WithUnit("1"))
	if err != nil {
		return err
	}
	completed.Add(ctx, podsCompleted)

	failed, err := p.Meter("eraser").SyncInt64().Counter("pods_failed_run_total", metric.WithDescription("total pods failed"), metric.WithUnit("1"))
	if err != nil {
		return err
	}
	failed.Add(ctx, podsFailed)

	jobTotal, err := p.Meter("eraser").SyncInt64().Counter("imagejob_run_total", metric.WithDescription("total number of imagejobs completed"), metric.WithUnit("1"))
	if err != nil {
		return err
	}
	jobTotal.Add(ctx, 1)
